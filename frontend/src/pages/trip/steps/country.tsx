      ...
      <div className="max-w-5xl mx-auto px-0 py-4 space-y-6">
        <div className="relative max-w-md mx-auto">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 rounded-2xl bg-white/5 dark:bg-zinc-900 border border-white/10 dark:border-zinc-700 text-white dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm hover:shadow-md transition-all"
          />
        </div>

        {loading && (
          <div className="text-center text-zinc-400 animate-pulse">Searching…</div>
        )}
        ...
        <div className="space-y-6 sm:grid sm:grid-cols-2 lg:grid-cols-3 sm:gap-6 sm:space-y-0">
          {countries.map((c) => (
            <div
              key={c._id}
              className="relative overflow-hidden rounded-3xl bg-white/5 dark:bg-zinc-800/20 border border-white/10 dark:border-zinc-700 shadow-sm hover:shadow-lg cursor-pointer transition-all duration-300 active:scale-[0.98]"
            >
              {c.images.length > 0 && (
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 space-y-2">
                <div className="text-lg font-semibold text-white dark:text-zinc-100">{c.name}</div>

                {c.description && (
                  <div className="text-sm text-zinc-300 dark:text-zinc-400 line-clamp-2">{c.description}</div>
                )}

                <button
                  onClick={() => selectCountry(c._id)}
                  className="mt-2 w-full py-2.5 rounded-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white text-sm font-semibold active:scale-[0.97] transition-all"
                >
                  Create Trip
                </button>
              </div>
        ...
        {!loading && countries.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 space-y-4">
            ...
            <p className="text-zinc-400 dark:text-zinc-400 text-center text-lg">{q.trim() ? "No countries found" : "Search for a country to begin"}</p>
          </div>
        )}
      </div>
    </TripStepLayout>
