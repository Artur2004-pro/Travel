...
const TripView: React.FC = () => {
  ...
  return (
    <div className="max-w-feed mx-auto pb-20 md:pb-10">
      <header className="sticky top-0 z-10 h-14 flex items-center justify-between border-b border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 px-4">
        <button
      ...
      <div className="px-4 py-6 space-y-6">
        {trip.coverImage && (
          <div className="aspect-square rounded-lg overflow-hidden bg-neutral-100 dark:bg-neutral-900">
            <img
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  <span className="flex items-center gap-1 text-xs text-neutral-500 dark:text-neutral-400">
                    <MapPin size={12} strokeWidth={2} /> {day.cityName}
                  </span>
                </div>
                  ...
                  {day.activities.map((act, i) => (
                    <div key={i} className="flex gap-2 items-start text-sm">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {act.notes && (
                          <p className="text-xs text-neutral-400 mt-1">{act.notes}</p>
                        )}
                        {act.cost != null && (
                          <p className="text-xs text-neutral-400">Cost: {act.cost}</p>
                        )}
                      </div>
                    </div>
                  ))}
